...
Same shape as Use Case 1's frontend. The only difference is what happens *behind* the agent invoke — UC2 has two OBO hops downstream where UC1 had one. This file just sends the user's JWT and renders the answer.

## What it does

- Serves a minimal HTML UI with "Sign in" and "Ask agent" actions.
- Runs the Entra authorization-code flow via MSAL.
...
|---|---|---|
| GET | `/` | Home page |
| GET | `/auth/login` | Kick off Entra sign-in to FrontendApp |
...
- `state` parameter on the OAuth redirect prevents CSRF on the callback.
- We store the raw access token in the session. In production, consider encrypting session contents at rest or using a server-side session store (Redis, etc.).
