"""Clean up resources created by the github-mcp-passthrough scripts.

The no-auth gateway (context7-gateway) is shared with the Context7 MCP lab, so
this script removes only this lab's own GitHub passthrough target. It deletes the
shared gateway and its IAM role only when no targets remain on the gateway (that
is, the Context7 lab has also been cleaned up). Tolerant of already-deleted
resources.
...
def main():
    ...
    # Only tear down the shared gateway when no targets are left (the Context7
    # lab has also been cleaned up).
