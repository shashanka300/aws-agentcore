"""Clean up resources created by the http-runtime-target scripts.

The gateway is shared with the A2A runtime-agent lab, so this script removes
only this lab's own target and OBO credential provider. It deletes the shared
gateway and its IAM role only when no targets remain on the gateway (that is,
the other lab has also been cleaned up). Tolerant of already-deleted resources.
Does not delete the AgentCore Runtime agent (remove that with
...
CRED_PROVIDER_NAME = "http-runtime-obo"
...
def main():
    ...
    print("--- Deleting OBO credential provider ---")
    try:
    ...
    # runtime-agent lab has also been cleaned up).
