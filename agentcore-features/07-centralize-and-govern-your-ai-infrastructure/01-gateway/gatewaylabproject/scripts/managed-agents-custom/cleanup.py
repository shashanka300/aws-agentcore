"""Clean up resources created by the managed-agents-custom scripts.

The gateway is shared with the A2A and HTTP runtime-agent labs, so this script
removes only this lab's own passthrough target. It deletes the shared gateway
and its IAM role only when no targets remain on the gateway (that is, the other
labs have also been cleaned up). There is no credential provider to delete (the
client supplies its own Claude key). Tolerant of already-deleted resources.
