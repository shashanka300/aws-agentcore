"""Clean up resources created by the databricks-a2a-target scripts.

The gateway is shared with the other runtime-agent labs, so this script removes
only this lab's own A2A passthrough target and its Databricks OAuth credential
provider. It deletes the shared gateway and its IAM role only when no targets
remain on the gateway. Tolerant of already-deleted resources. Does not delete
the Databricks App or service principal (remove those in the Databricks console).
...
CRED_PROVIDER_NAME = "databricks-a2a-oauth"
...
def main():
    ...
    cred_name = os.environ.get("CREDENTIAL_PROVIDER_NAME", CRED_PROVIDER_NAME)
    ...
    print("--- Deleting Databricks OAuth credential provider ---")
    try:
        admin.delete_credential_provider(cred_name)
    except Exception as e:  # noqa: BLE001
        print(f"  Error: {e}")

    # Only tear down the shared gateway when no targets are left (the other
    # runtime-agent labs have also been cleaned up).
