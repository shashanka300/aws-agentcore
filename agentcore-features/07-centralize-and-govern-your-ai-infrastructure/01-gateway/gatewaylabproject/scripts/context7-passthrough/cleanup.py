"""Clean up resources created by the context7-passthrough scripts.

The no-auth gateway (context7-gateway) is shared with the GitHub MCP lab, so this
script removes only this lab's own Context7 passthrough target. It deletes the
shared gateway and its IAM role only when no targets remain on the gateway (that
is, the GitHub lab has also been cleaned up). Tolerant of already-deleted
resources.
...
def main():
    ...
    # Only tear down the shared gateway when no targets are left (the GitHub MCP
