"""
utils.py — shared helpers for the pay-for-x402-secure-data notebook.

Small wrappers around boto3 for pretty-printing responses, assuming IAM
roles, polling for status transitions, and tolerating idempotent create
calls. Mirrors the helper surface used by the sibling ``pay-for-api-agent``
use case so the two notebooks read the same way.
"""
...
def assume_role(
    ...
    role_arn: str,
    session_name: str = "x402-secure-data-session",
) -> boto3.Session:
    """Assume an IAM role and return a boto3 Session with auto-refreshing credentials.

    Uses botocore's ``RefreshableCredentials`` so the returned session stays
    valid past the default 1-hour STS expiry without the caller rebuilding
    clients. This matters for the notebook, where a user can leave a setup
    cell sitting for hours before coming back to the paid run or the data-plane
    inspection cells.

    Immediately verifies the assumed identity by calling get_caller_identity();
    raises if the assumption fails outright.
    """
...
def write_env_updates(updates: dict, env_path: str = ".env") -> None:
    ...
    (no quoting), matching the existing .env style in this use case.

    Used only for non-secret values written by the notebook at runtime
    (USER_ID, role ARNs, manager ARN, connector ID, instrument ID, session
    ID, wallet address). Wallet-provider secrets (Coinbase CDP keys) are
    pasted into ``.env`` by the user manually and never flow through this
    function. After the notebook calls ``CreatePaymentCredentialProvider``,
    AgentCore Identity stores those secrets in AWS Secrets Manager under KMS
    encryption and only the credential-provider ARN remains in ``.env``. The
    ``.env`` file itself is gitignored from use-case creation.
    """
    import pathlib
