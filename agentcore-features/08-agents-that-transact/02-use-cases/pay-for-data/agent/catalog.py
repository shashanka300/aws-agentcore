...
def fetch_live_catalog(
    ...
    Args:
        ...
        catalog_url: Override the catalog URL. When provided, skips get_config()
                     entirely — useful for host-side scripts that don't have
                     payment env vars set yet.
    """
    if catalog_url is None:
        cfg = get_config()
        catalog_url = cfg.heurist_catalog_url
    http = session or requests.Session()
    response = http.get(catalog_url, timeout=30, stream=True)
