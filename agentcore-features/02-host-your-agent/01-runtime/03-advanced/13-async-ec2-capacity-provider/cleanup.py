...
them has been idle for `idleInstanceTimeout` (60s in this sample), but the
CapacityProvider itself persists until you delete it.

And this is the sample where "idle" is not automatic. The agent's `/ping` handler
returns HealthyBusy while an async task is active, which suppresses the idle
timer by design. An async task that is never completed — an exception on a path
without a `finally`, say — leaves `active_count` above zero forever, so the
instance is never reclaimed and never stops billing. Deleting the
CapacityProvider is the backstop for exactly that.
