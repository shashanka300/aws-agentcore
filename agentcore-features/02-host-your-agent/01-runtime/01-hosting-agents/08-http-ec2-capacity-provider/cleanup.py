...
and never lists-and-deletes, which matters if you share an account — nothing
outside your own config is touched.
...
def delete_capacity_provider(agentcore, config, attempts: int = 20) -> None:
    ...
    for _ in range(120):
        ...
        time.sleep(15)
    print(f"  still not deleted after 30 min. Check for running instances, then "
...
def delete_ecr_repo(region: str, repo: str) -> None:
    ...
    try:
        ...
        print(f"  ECR repo {repo}: {type(exc).__name__}: {exc}")


def delete_codebuild_project(region: str, project: str) -> None:
    cb = boto3.client("codebuild", region_name=region)
    try:
        cb.delete_project(name=project)
        print(f"  deleted CodeBuild project: {project}")
    except Exception as exc:  # noqa: BLE001
        print(f"  CodeBuild project {project}: {type(exc).__name__}: {exc}")
...
def main() -> None:
    ...
    # The region deploy.py recorded, else the environment. Passed on to the
    # S3/ECR/CodeBuild clients below, so it has to be a real value, not None.
    ...
    for label, key in (
        ...
        ("IAM CodeBuild role", "codeBuildRole"),
    ):
        if config.get(key):
    ...
    print("Build and artifact resources:")
    if config.get("bucket"):
        empty_and_delete_bucket(region, config["bucket"])
    if config.get("ecrRepository"):
        delete_ecr_repo(region, config["ecrRepository"])
    if config.get("codeBuildProject"):
        delete_codebuild_project(region, config["codeBuildProject"])

    print("IAM roles:")
    for key in ("runtimeRole", "codeBuildRole"):
        if config.get(key):
            delete_role(config[key])
