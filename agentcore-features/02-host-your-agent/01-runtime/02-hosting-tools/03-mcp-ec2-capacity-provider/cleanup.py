...
def main() -> None:
    ...
    CONFIG.unlink()
    # invoke.py's cached session id, which points at a runtime that no longer
    # exists. Left behind it would only be ignored, but it is ours to clean up.
    (HERE / ".mcp_session.json").unlink(missing_ok=True)
